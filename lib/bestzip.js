...
function nativeZip(dest, source, opts, done) {
    if (done === undefined) {
		done = opts;
		opts = {args: []};
	}
    ...
    var args = opts.args.concat(['--quiet', '--recurse-paths', dest]).concat(source);
    cp.spawn('zip', args, opts.spawn)
...
function zip(dest, sources, opts, done) {
    ...
    which(function(err, which) {
        ...
        if (which == NATIVE) {
            nativeZip(dest, sources, opts, done);
        } else {
